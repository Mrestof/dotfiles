...
import catppuccin  # package: https://github.com/catppuccin/qutebrowser
...
# TODO: play with thresholds below when you find pages working bad with existing
#       values.
...
# disable adblock for all sites that detect it or for ones I don't mind use it
adblock_excluded_sites = (
    'www.wcofun.net',  # the site detects adblock and stops until you disable it
)
for url in adblock_excluded_sites:
    config.set('content.blocking.enabled', False, url)
...
# enable clipboard paste access for trusted sites
js_clipboard_trusted_sites = (
    'github.com',
    'youtube.com',
)
for url in js_clipboard_trusted_sites:
    config.set('content.javascript.clipboard', 'access-paste', url)
...
c.editor.command = [
    environ['TERMCMD'], '-e',
    environ['EDITOR'], '-f', '{file}', '-c', 'normal {line}G{column0}l'
]
...
# userscripts
config.bind(',i', 'hint images userscript swayimg-view')
config.bind(',q', 'spawn --userscript /usr/share/qutebrowser/userscripts/qr')
config.bind(',t', 'spawn --userscript translate')
config.bind(',c', 'spawn --userscript copy-page-html')

config.bind(',,', 'spawn --userscript copy-num')


# custom binds
config.bind('cc', 'config-source')
config.bind(',yr', 'hint links yank --rapid')
